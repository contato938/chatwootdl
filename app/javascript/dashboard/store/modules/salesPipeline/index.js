/* global axios */
import types from 'dashboard/store/mutation-types';

const state = {
  salesPipeline: null,
  stages: [],
  kanbanData: [],
  uiFlags: {
    isFetching: false,
    isUpdating: false,
    isCreating: false,
    isDeleting: false,
  },
};

const getters = {
  getSalesPipeline: $state => $state.salesPipeline,
  getStages: $state => $state.stages,
  getKanbanData: $state => $state.kanbanData,
  getUIFlags: $state => $state.uiFlags,
  getDefaultStage: $state => $state.stages.find(stage => stage.is_default),
  getStageById: $state => stageId => $state.stages.find(stage => stage.id === stageId),
};

const actions = {
  async fetchSalesPipeline({ commit }, { accountId }) {
    commit(types.SET_SALES_PIPELINE_UI_FLAG, { isFetching: true });
    try {
      const response = await axios.get(
        `/api/v1/accounts/${accountId}/sales_pipelines`
      );
      commit(types.SET_SALES_PIPELINE, response.data);
      commit(types.SET_SALES_PIPELINE_STAGES, response.data.stages || []);
      return response.data;
    } catch (error) {
      throw error;
    } finally {
      commit(types.SET_SALES_PIPELINE_UI_FLAG, { isFetching: false });
    }
  },

  async fetchKanbanData({ commit }, { accountId, filters = {} }) {
    commit(types.SET_SALES_PIPELINE_UI_FLAG, { isFetching: true });
    try {
      const response = await axios.get(
        `/api/v1/accounts/${accountId}/sales_pipelines/kanban`,
        { params: filters }
      );
      commit(types.SET_KANBAN_DATA, response.data);
      return response.data;
    } catch (error) {
      throw error;
    } finally {
      commit(types.SET_SALES_PIPELINE_UI_FLAG, { isFetching: false });
    }
  },

  async createStage({ commit, dispatch }, { accountId, stageData }) {
    commit(types.SET_SALES_PIPELINE_UI_FLAG, { isCreating: true });
    try {
      const response = await axios.post(
        `/api/v1/accounts/${accountId}/sales_pipeline_stages`,
        { stage: stageData }
      );
      commit(types.ADD_SALES_PIPELINE_STAGE, response.data);
      await dispatch('fetchSalesPipeline', { accountId });
      return response.data;
    } catch (error) {
      throw error;
    } finally {
      commit(types.SET_SALES_PIPELINE_UI_FLAG, { isCreating: false });
    }
  },

  async updateStage({ commit, dispatch }, { accountId, stageId, stageData }) {
    commit(types.SET_SALES_PIPELINE_UI_FLAG, { isUpdating: true });
    try {
      const response = await axios.put(
        `/api/v1/accounts/${accountId}/sales_pipeline_stages/${stageId}`,
        { stage: stageData }
      );
      commit(types.UPDATE_SALES_PIPELINE_STAGE, response.data);
      await dispatch('fetchSalesPipeline', { accountId });
      return response.data;
    } catch (error) {
      throw error;
    } finally {
      commit(types.SET_SALES_PIPELINE_UI_FLAG, { isUpdating: false });
    }
  },

  async deleteStage({ commit, dispatch }, { accountId, stageId, migrationStageId }) {
    commit(types.SET_SALES_PIPELINE_UI_FLAG, { isDeleting: true });
    try {
      await axios.delete(
        `/api/v1/accounts/${accountId}/sales_pipeline_stages/${stageId}`,
        { data: { stage: { migration_stage_id: migrationStageId } } }
      );
      commit(types.REMOVE_SALES_PIPELINE_STAGE, stageId);
      await dispatch('fetchSalesPipeline', { accountId });
    } catch (error) {
      throw error;
    } finally {
      commit(types.SET_SALES_PIPELINE_UI_FLAG, { isDeleting: false });
    }
  },

  async reorderStages({ commit, dispatch }, { accountId, stages }) {
    commit(types.SET_SALES_PIPELINE_UI_FLAG, { isUpdating: true });
    try {
      await axios.put(
        `/api/v1/accounts/${accountId}/sales_pipeline_stages/reorder`,
        { stages }
      );
      commit(types.REORDER_SALES_PIPELINE_STAGES, stages);
      await dispatch('fetchSalesPipeline', { accountId });
    } catch (error) {
      throw error;
    } finally {
      commit(types.SET_SALES_PIPELINE_UI_FLAG, { isUpdating: false });
    }
  },

  async updateConversationStage({ commit }, { accountId, conversationId, stageId }) {
    try {
      const response = await axios.put(
        `/api/v1/accounts/${accountId}/conversations/${conversationId}/sales_stage`,
        { sales_stage: { stage_id: stageId } }
      );
      return response.data;
    } catch (error) {
      throw error;
    }
  },

  async removeConversationStage({ commit }, { accountId, conversationId }) {
    try {
      await axios.delete(
        `/api/v1/accounts/${accountId}/conversations/${conversationId}/sales_stage`
      );
    } catch (error) {
      throw error;
    }
  },

  async fetchConversationStage({ commit }, { accountId, conversationId }) {
    try {
      const response = await axios.get(
        `/api/v1/accounts/${accountId}/conversations/${conversationId}/sales_stage`
      );
      return response;
    } catch (error) {
      throw error;
    }
  },
};

const mutations = {
  [types.SET_SALES_PIPELINE_UI_FLAG]($state, uiFlag) {
    $state.uiFlags = {
      ...$state.uiFlags,
      ...uiFlag,
    };
  },

  [types.SET_SALES_PIPELINE]($state, data) {
    $state.salesPipeline = data;
  },

  [types.SET_SALES_PIPELINE_STAGES]($state, stages) {
    $state.stages = stages.sort((a, b) => a.position - b.position);
  },

  [types.SET_KANBAN_DATA]($state, data) {
    $state.kanbanData = data;
  },

  [types.ADD_SALES_PIPELINE_STAGE]($state, stage) {
    $state.stages.push(stage);
    $state.stages.sort((a, b) => a.position - b.position);
  },

  [types.UPDATE_SALES_PIPELINE_STAGE]($state, updatedStage) {
    const index = $state.stages.findIndex(stage => stage.id === updatedStage.id);
    if (index !== -1) {
      $state.stages.splice(index, 1, updatedStage);
    }
  },

  [types.REMOVE_SALES_PIPELINE_STAGE]($state, stageId) {
    $state.stages = $state.stages.filter(stage => stage.id !== stageId);
  },

  [types.REORDER_SALES_PIPELINE_STAGES]($state, stages) {
    stages.forEach(({ id, position }) => {
      const stage = $state.stages.find(s => s.id === id);
      if (stage) {
        stage.position = position;
      }
    });
    $state.stages.sort((a, b) => a.position - b.position);
  },
};

export default {
  namespaced: true,
  state,
  getters,
  actions,
  mutations,
};
